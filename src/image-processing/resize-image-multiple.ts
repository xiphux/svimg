...
export default async function resizeImageMultiple(
  inputFile: string,
  outputDir: string,
  queue: Queue,
  options: ResizeImageMultipleOptions,
): Promise<Image[]> {
  if (!inputFile) {
  ...
  const widthPaths: Array<{ path: string; width: number }> = options.widths.map(
    (width) => {
  ...
  return options?.skipGeneration
    ? widthPaths.map(({ path, width }) => ({
        ...
        width,
        height:
          Math.round((width / options.aspectRatio + Number.EPSILON) * 100) /
          100,
      }))
    : await Promise.all(
        widthPaths.map(({ width, path }) =>
          ensureResizeImage(inputFile, path, queue, {
            width,
            quality: options.quality,
          }),
        ),
